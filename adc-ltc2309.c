//------------------------------------------------------------------------------------------------------------
//
// I2C ADC LTC2309 library. (Use 8 Single-Ended, Unipolar Mode)
//
//------------------------------------------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
...
int check_adc (int fd)
{
	...
	for (i = 0; i < ARRARY_SIZE(ADC_ADDR); i++) {
		...
		if(i2c_read_word(fd, ADC_ADDR[i]) < 0) {
			info("Not detect %s ADC%d(Device Addr : 0x%02x)\n",
